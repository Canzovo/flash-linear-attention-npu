...
已有 `ChunkGatedDeltaRuleFwdH` 未经管理账号确认修改算子原型并造成用户兼容性问题。算子原型、aclnn 接口和 `fla_npu.ops.ascendc.<op_name>` 接口一旦确定，必须作为兼容性契约保持稳定。除非仓库管理账号 `@weinachuan` 针对本次具体变更明确下达命令，任何开发或修复都不得修改以下内容：
