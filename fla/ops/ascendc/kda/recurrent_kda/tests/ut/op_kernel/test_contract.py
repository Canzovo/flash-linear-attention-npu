...
def test_kernel_validates_device_metadata_before_state_access():
    ...
    slot_resolution = text.index(
        "uint64_t stateSlot = ResolveInitialStateSlot(batch_i, seq0, seqLen)"
    )
    process_head = text.index("statePrefetched = ProcessHead(")
    assert empty_skip < slot_validation < slot_resolution < process_head

    process_head_body = text[text.index("__aicore__ inline bool ProcessHead"):]
    assert "PrefetchState(stateSlot, head_i, nextVOffset, nextSingleV)" in process_head_body
    assert "PrefetchState(nextStateSlot, nextHead, 0, nextSingleV)" in process_head_body
