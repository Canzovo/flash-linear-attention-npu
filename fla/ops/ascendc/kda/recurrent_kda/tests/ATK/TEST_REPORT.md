...
- 性能范围：设备耗时 `13.0586`～`505.8273 us`，MFU `2.573007%`～`4.959628%`，MBU `34.877654%`～`66.747519%`。
...
|---:|---:|---|---:|---:|---:|
| 0 | 1 | base | 13.0586 | 2.618404 | 35.492976 |
| 1 | 1 | cb_mtp | 13.2890 | 2.573007 | 34.877654 |
| 2 | 4 | base | 31.9610 | 4.279302 | 57.674621 |
| 3 | 4 | cb_mtp | 31.9865 | 4.275891 | 57.628714 |
| 4 | 16 | base | 110.3073 | 4.959628 | 66.747519 |
| 5 | 16 | cb_mtp | 110.6434 | 4.944562 | 66.544843 |
| 6 | 64 | base | 505.8273 | 4.326244 | 58.202352 |
| 7 | 64 | cb_mtp | 505.0225 | 4.333139 | 58.295175 |

## MFU / MBU 口径

- 逻辑 FLOPs 下界：`5 × B × H × T × K × V`；其中 `T` 为解码步长（本次 `T=S=1`），`H=96`、`K=V=128`。该口径是算法级算术下界，不计 exp/sigmoid 等超越函数。
- 逻辑读流量为所有输入张量各读一次；逻辑写流量为输出加原地状态写回。该口径是算法流量模型，不是 HBM 硬件计数器实测字节数。
- `MFU = (逻辑 FLOPs / Device 耗时) / 23 TFLOPS × 100%`。
- `MBU = (逻辑读写字节 / Device 耗时) / 1.4 TB/s × 100%`。
- 本次按 `Ascend950PR` 标称峰值计算：算力 `23 TFLOPS`、带宽 `1.4 TB/s`。
