...
def _make_non_contiguous_state(torch, tensor):
...
def test_non_contiguous_state_cases():
    ...
    for case in cases:
        ...
        initial_state, state_pool, state_guard = _make_non_contiguous_state(
            torch, inputs["initial_state"].to(device)
        )
        initial_stride = initial_state.stride()
        initial_storage = initial_state.untyped_storage().data_ptr()
        out, final_state = recurrent_kda(
            inputs["q"].to(device),
            inputs["k"].to(device),
            inputs["v"].to(device),
        ...
        assert initial_state.stride() == initial_stride
        assert initial_state.untyped_storage().data_ptr() == initial_storage
        assert torch.equal(state_pool[:, 1].cpu(), state_guard.cpu())
        if attrs.get("output_final_state", False):
            assert final_state.data_ptr() == initial_storage
            assert final_state.stride() == initial_stride
