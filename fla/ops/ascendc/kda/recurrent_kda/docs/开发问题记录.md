...
## Issue 07：arch35 不支持 BF16 标量直接转换为 FP32
...
![BF16 标量转换编译错误](images/issue-07-bf16-scalar-cast.png)

### 根因分析

候选优化尝试从 GM 直接读取单个 beta，并对 `bfloat16_t` 执行标量 `static_cast<float>`。当前 Ascend950 编译器后端不支持该标量转换；若改为 GM→UB 后使用 Vector Cast，则需要额外 MTE2、跨 pipe 同步和 Vector 指令。

### 解决办法

使用 UB Cast 后候选可以编译并通过精度，但相同 case 性能为 21.365 us，劣于直接任务步进版本，因此完整回退单 beta 读取候选，保留原批量 beta 搬运和转换路径。

### 验证结果

回退后 Ascend950 单算子 run 包重新编译成功，最终精度测试通过。

## Issue 08：候选回退时 beta 初始化调用落入不可达分支
...
![候选回退后的精度错误](images/issue-08-copyinbeta-rollback.png)

### 根因分析

反向应用候选补丁时，`CopyInBeta(seq0, seq1)` 被放入 `stateSlot == INVALID_STATE_SLOT` 分支，并位于 `return` 之后。有效任务路径因此没有初始化 beta UB，造成输出和 final state 大面积精度错误。

### 解决办法

恢复花括号边界，将 `CopyInBeta` 放回 state slot 校验成功后的有效路径；重新构建 run 包并在 wheel 安装后最后安装该 run 包，确保新 Python 进程命中最终 binary。

### 验证结果

修正后 `test_accuracy.py` 的 5 个场景中，`out` 和 `final_state` 全部 PASS。
