...
golden/Triton tensors instead of running CT online.
...
def run_ct_l1_dual(
    ...
    for name in TENSOR_RESULT_NAMES:
        ...
        if not isinstance(gt, torch.Tensor) or not isinstance(bench, torch.Tensor):
            raise TypeError(f"golden/Triton {name} must both be Tensor values")
        print(f"  CT dual L1: case={case_name}, tensor={name}", flush=True)
        results[name] = dual(
